...
This module is designed to take a list of shell commands and automatically
add some common desirable functionality to them including:
...
%options is a hash of (OPTION => VALUE).  The following OPTIONS exist:

OPTION = run
------------

The 'run' option determines how the commands will be handled by the run
...
you didn't know which of curl, wget, or lftp were available, you might
...
fail if all three alternates fail.  Of course, it is likely you would
need to add other options to the three commands to get similar behavior
from the three alternative, but this example illustrates the use of
alternates.
...
'flow' option for all parts of it so it so it will handle command
echoing, error handling, and output handling correctly.  For example,
you need to do:
